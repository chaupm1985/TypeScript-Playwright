...
test("Ensure  proper error handling when mandoroty field are blank",async({page}) =>{
    ...
    await tc_cartPage.addressTextbox.fill(" ");
    await tc_cartPage.firstnameTextbox.fill(" ");
    await tc_cartPage.lastnameTextbox.fill(" ");
    await tc_cartPage.placeorderButton.click();
    await expect(tc_cartPage.errorfname).toBeVisible();
    await expect(tc_cartPage.errorlname).toBeVisible();
    await expect(tc_cartPage.errorstreet).toBeVisible();
})